...
const {exec, dockerImage, createTimestamp} = require("./util");
...
exports.saveVolume = async (volume, filename, overwrite = false) => {
  ...
  await exec(`docker run --rm -w /from -v ${volume}:/from -v ${path.dirname(filename)}:/to ${dockerImage} tar -czvf /to/${path.basename(filename)} .`);
};
...
exports.loadVolume = async (filename, volume, overwrite = false) => {
  ...
  await exec(`docker run --rm -w /to -v ${volume}:/to -v ${filename}:/from/volume.tgz ${dockerImage} tar -xzvf /from/volume.tgz`);
};
